...
if nargin < 3
    ...
    [s_transport, T126_evol, T116_evol, T216_evol, T226_evol, ...
     T346_evol, T336_evol, T436_evol, T446_evol] = ...
        track_second_order_evolution(beamline, 0.001,E0);
    ...
    for idx = 1:length(valid_slice_indices)
        ...
        for i = 2:num_exit_points
            ...
            % 逐元件累积传输矩阵，同时跟踪 gamma
            temp_gamma = gamma0 * (1 + delta0_slice);
            for j = 1:length(current_beamline_segment)
                elem = current_beamline_segment(j);
                
                if strcmpi(elem.type, 'cavity')
                    % Cavity 需要传入当前 gamma
                    R_elem = getElementMatrix(elem, temp_gamma);
                    
                    % 更新 gamma
                    if isfield(elem, 'num_cells') && isfield(elem, 'frequency')
                        elem.length = elem.num_cells * 0.5 * 3e8 / (elem.frequency * 1e6);
                    end
                    A = 0; if isfield(elem, 'gradient'), A = elem.gradient * 1e6; end
                    phi = 0; if isfield(elem, 'phase'), phi = elem.phase * pi/180; end
                    energy_gain = A * elem.length * cos(phi);
                    temp_gamma = temp_gamma + energy_gain / me_c2;
                else
                    % 其他元件不需要 gamma
            ...
            switch element_types(i)
                ...
                case 2 % Quadrupole
                    ...
                    if calc_k1_flag && slice_idx == valid_slice_indices(1)
                        k1_values = [k1_values, k1_nominal];
                    end
    ...
    for idx = 1:length(valid_slice_indices)
        ...
        dx = x_c - mean_x_centroid; dpx = px_c ;
        dy = y_c - mean_y_centroid; dpy = py_c;
